...
**Example Request:**
```bash
GET /repos?page=1&per_page=50&dependency_names=react,typescript
```
...
**Endpoint:** `GET /repos/{owner}/{name}/commits`

**Description:** Retrieves a paginated list of commits for a specific repository.
...
**Example Request:**
```bash
GET /repos/octocat/Hello-World/commits?page=1&per_page=50
```
...
**Pagination:** This endpoint uses page-based pagination with detailed pagination information in the response.
...
### 4. Get Repository Dependencies

**Endpoint:** `GET /repos/{owner}/{name}/dependencies`

**Description:** Retrieves a paginated list of dependencies used in a specific repository with cursor-based pagination and sorting options.
...
| `per_page` | integer | No | 30 | Number of items per page (max: 100) |
| `sort_by` | string | No | dependency_id | Field to sort by (dependency_id, first_use_date, last_use_date) |
| `order` | string | No | asc | Sort order: "asc" or "desc" |

**Example Request:**
```bash
GET /repos/octocat/Hello-World/dependencies?per_page=100&sort_by=first_use_date&order=desc
```
...
**Pagination:** This endpoint uses page-based pagination. Use the `next_page` value from the response as the `page` parameter for the next page.

**Sorting Options:**
- `dependency_id` - Sort by dependency ID (default)
- `first_use_date` - Sort by first use date
- `last_use_date` - Sort by last use date
